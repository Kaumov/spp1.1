...
class WebServerRequestHandler(http.server.BaseHTTPRequestHandler):
        def do_GET(self):
                if self.path == '/':
                        ...
                        self.send_header("Location", "https://oauth.vk.com/authorize?client_id=5341594&display=page&redirect_uri=http://localhost:12346/verify&scope=video,wall,widgets&response_type=code&v=5.45")
                        self.end_headers()
...
if __name__ == "__main__":
        ...
        print(wc.get("api.vk.com","/method/video.get?owner_id=291226449&need_user=1&count=7&v=5.45&access_token=%s" % at))
        print("___________________________________________")
        print(wc.get("api.vk.com","/method/widgets.get?user_id=291226449&count=3&count=1&sort=1&access_token=%s" % at))
        print("___________________________________________")
